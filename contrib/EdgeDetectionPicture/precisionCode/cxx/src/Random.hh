...
public:
    ...
    static const unsigned  int32_t u_int32_max = 4294967295u;
    ...
    // Set the seed.
    // If zero, seed from the system clock.
    void reseed (unsigned int64_t seed);
    ...
    inline unsigned int32_t ui32 ();                    
    inline unsigned int32_t ui32 (u_int32_t a, u_int32_t b);  

protected:

    void _init (unsigned int64_t seed);

    // The original seed for this random stream.
    unsigned int64_t 	_seed;

    // The current state for this random stream.
    unsigned int16_t 	_xsubi[3];

};

inline unsigned int32_t 
Random::ui32 () 
{ 
