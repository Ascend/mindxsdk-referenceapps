...
class Vgg16(nn.Cell):
    ...
    def __init__(self, requires_grad=False):
        ...
        net = build_vgg16()
        params = load_checkpoint('./model/vgg16_ascend_v180.ckpt')
        load_param_into_net(net, params)
        vgg_pretrained_features = net.layers
...
class Vgg(nn.Cell):
    ...
    def __init__(self, base, num_classes=1000, batch_norm=False, args=None, phase="train",
                 include_top=True):
...
def build_vgg16(num_classes=1000, phase="train", **kwargs):
    """
    ...
    args = Namespace(
        ...
        pad_mode='pad',
        has_bias=False)
    base = cfg.get('16', None)
    assert base is not None
    net = Vgg(base, num_classes=num_classes, args=args, phase=phase, **kwargs)
    return net
