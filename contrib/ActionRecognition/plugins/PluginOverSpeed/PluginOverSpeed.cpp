...
namespace {
    const int minframe = 4;
}

namespace MxPlugins {
    APP_ERROR PluginOverSpeed::Init(std::map<std::string, std::shared_ptr<void>> &configParamMap) {
        LogInfo << "PluginOverSpeed::Init start.";
        APP_ERROR ret = APP_ERR_OK;
        // Get the property values by key
        std::shared_ptr<std::string> tracksourcePropSptr =
                std::static_pointer_cast<std::string>(configParamMap["dataSourceTrack"]);
        tracksource_ = *tracksourcePropSptr.get();

        std::shared_ptr<std::string> dataSourceDetection =
                std::static_pointer_cast<std::string>(configParamMap["dataSourceDetection"]);
        detectionsource_ = *dataSourceDetection.get();

        std::shared_ptr<std::string> descriptionMessageProSptr =
                std::static_pointer_cast<std::string>(configParamMap["descriptionMessage"]);
        descriptionMessage_ = *descriptionMessageProSptr.get();

        // Configuration parameter
        confthresh_ = *std::static_pointer_cast<int>(configParamMap["speedThresh"]);
    ...
    APP_ERROR PluginOverSpeed::DeInit() {
        LogInfo << "PluginOverSpeed::DeInit end.";
        return APP_ERR_OK;
    }

    APP_ERROR PluginOverSpeed::SetMxpiErrorInfo(MxpiBuffer &buffer, const std::string pluginName,
    ...
    APP_ERROR PluginOverSpeed::Process(std::vector<MxpiBuffer *> &mxpiBuffer) {
        LogInfo << "PluginOverSpeed::Process start";
        ...
        if (errorInfoPtr != nullptr) {
            ErrorInfo_ << GetError(APP_ERR_COMM_FAILURE, pluginName_)
                       << "PluginOverSpeed process is not implemented";
            mxpiErrorInfo.ret = APP_ERR_COMM_FAILURE;
            mxpiErrorInfo.errorInfo = ErrorInfo_.str();
            SetMxpiErrorInfo(*buffer, pluginName_, mxpiErrorInfo);
            LogError << "PluginOverSpeed process is not implemented";
        ...
        if (sleeptime == 0) {
            int alarm = calculate(trackdata, confframes_, frame_num, confthresh_,
                                  srcTrackLetListSptr,
                                  srcObjectListSptr);
            if (alarm == 1) {
                alarm_count++;
                ...
                mxpiAttribute->set_attrname("Alarm OverSpeed");
                APP_ERROR ret = mxpiMetadataManager.AddProtoMetadata(pluginName_,
                                                                     static_pointer_cast<void>(result));
                if (ret != APP_ERR_OK) {
                    ErrorInfo_ << GetError(ret, pluginName_) << "PluginOverSpeed add metadata failed.";
        ...
        SendData(0, *buffer);
        LogInfo << "PluginOverSpeed::Process end";
        return APP_ERR_OK;
    }

    int PluginOverSpeed::calculate(std::map<int, std::vector<int>> &trackdata, int confframes_, int &frame_num,
                                   int confthresh_,
        ...
        for (uint32_t i = 0; i < (uint32_t) srcTrackLetListSptr->trackletvec_size(); i++) {
            ...
            if (trackdata.count(trackObject.trackid()) == 0) {
                ...
                trackdata[trackObject.trackid()][2] = 0;
                trackdata[trackObject.trackid()][3] = 0;
            } else {
                trackdata[trackObject.trackid()][2] += 1;
                int x0 = trackdata[trackObject.trackid()][0];
                int y0 = trackdata[trackObject.trackid()][1];
                int x1 = (int) ((detectObject.x0() + detectObject.x1()) / 2);
                int y1 = (int) ((detectObject.y0() + detectObject.y1()) / 2);
                trackdata[trackObject.trackid()][3] += distance(x0, y0, x1, y1);
        ...
        if (frame_num == confframes_) {
            ...
            for (auto iter = trackdata.begin(); iter != trackdata.end(); iter++) {
                if (trackdata[iter->first][2] == 0 || trackdata[iter->first][2] < minframe) {
                    continue;
                }
                if (((int) (trackdata[iter->first][3] / trackdata[iter->first][2])) > confthresh_) {
                    alarm = 1;
                    break;
                }
            }
    ...
    std::vector<std::shared_ptr<void>> PluginOverSpeed::DefineProperties() {
        // Define an A to store properties
        std::vector<std::shared_ptr<void>> properties;
        // Set the type and related information of the properties, and the key is the name
        auto tracksourceProSptr =
                std::make_shared<ElementProperty<std::string>>(ElementProperty<std::string>{
                        STRING, "dataSourceTrack", "name",
                        "the source of tack data", "mxpi_motsimplesort0",
                        "NULL", "NULL"});

        auto detectsourceProSptr =
                std::make_shared<ElementProperty<std::string>>(ElementProperty<std::string>{
                        STRING, "dataSourceDetection", "name",
                        "the source of detect data", "mxpi_objectpostprocessor0",
                        "NULL", "NULL"});

        auto threshProSptr =
                std::make_shared<ElementProperty<int>>(ElementProperty<int>{
                        UINT, "speedThresh", "thresh",
                        "the thresh of overspeed", 10,
                        1, 1000});

        auto frameProSptr =
                std::make_shared<ElementProperty<int>>(ElementProperty<int>{
                        UINT, "frames", "frame",
                        "the number of frame when judging", 8,
                        8, 100});

        auto sleepProSptr =
                std::make_shared<ElementProperty<int>>(ElementProperty<int>{
                        UINT, "detectSleep", "sleep",
                        "the time of stop detection", 8, 0,
                        300});
        ...
        auto descriptionMessageProSptr =
                std::make_shared<ElementProperty<std::string>>(ElementProperty<std::string>{
                        ...
                        "This is PluginOverSpeed", "NULL", "NULL"});
    ...
    MxpiPortInfo PluginOverSpeed::DefineInputPorts() {
    ...
    MxpiPortInfo PluginOverSpeed::DefineOutputPorts() {
    ...
    int PluginOverSpeed::distance(int x0, int y0, int x1, int y1) {
        int distance = int(sqrt((x1 - x0) * (x1 - x0) + (y1 - y0) * (y1 - y0)));
        return distance;
    }
}
namespace {
    MX_PLUGIN_GENERATE(PluginOverSpeed)
}
