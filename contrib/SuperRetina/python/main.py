...
def image_read(read_query_path, read_refer_path, query_is_image=False):
    if query_is_image:
        query_image = read_query_path
    else:
        query_image = cv2.imread(read_query_path, cv2.IMREAD_COLOR)
        query_image = query_image[:, :, 1]
        query_image = pre_processing(query_image)
    refer_image = cv2.imread(read_refer_path, cv2.IMREAD_COLOR)

    assert query_image.shape[:2] == refer_image.shape[:2]
    read_image_height, read_image_width = query_image.shape[:2]
    ...
    return query_image, refer_image, read_image_height, read_image_width
...
def evaluate(query_path, refer_path, infer_model, query_is_image=False):
    raw_tensor_query , raw_tensor_refer, eval_image_height, eval_image_width = image_read(query_path, refer_path, query_is_image=query_is_image)
...
def match(query_path, refer_path, match_model, query_is_image=False):
    ...
    try:
        ...
        pass
    return goodMatch, cv_kpts_query, cv_kpts_refer, raw_query_image, raw_refer_image, match_image_height, match_image_width
