...
def image_read(query_path, refer_path, query_is_image=False):
    if query_is_image:
    ...
    image_height, image_width = query_image.shape[:2]
    ...
    return query_image, refer_image, image_height, image_width

def pool2d(A, kernel_size, stride, padding=0):
...
def sample_keypoint_desc(keypoints, descriptors, s: int = 8):
    ...
    keypoints /= np.array([w * s - 1, h * s - 1]).astype(np.float32)[None]
...
def evaluate(query_path, refer_path, model, query_is_image=False):
    raw_tensor_query , raw_tensor_refer, image_height, image_width = image_read(query_path, refer_path, query_is_image=query_is_image)
    ...
    outputs = model.infer(inputs_tensor)
    outputs[0].to_host()
    outputs[1].to_host()
    ...
    keypoints, scores = list(zip(*[
        remove_borders(np.array(k), np.array(s), 4, h, w)
    ...
    return keypoints, descriptors, raw_tensor_query, raw_tensor_refer, image_height, image_width

def match(query_path, refer_path, model, query_is_image=False):
    keypoints, descriptors, raw_query_image, raw_refer_image, image_height, image_width =\
    evaluate(query_path, refer_path, model, query_is_image=query_is_image)
    ...
    try:
        ...
        pass
    return goodMatch, cv_kpts_query, cv_kpts_refer, raw_query_image, raw_refer_image, image_height, image_width

def compute_homography(query_path, refer_path, model, query_is_image=False):
    goodMatch, cv_kpts_query, cv_kpts_refer, raw_query_image, raw_refer_image, image_height, image_width = \
    match(query_path, refer_path, model, query_is_image=query_is_image)
    H_m = None
    inliers_num_rate = 0
    ...
    return H_m, inliers_num_rate, raw_query_image, raw_refer_image, image_height, image_width
...
if __name__ == '__main__':
    # init stream manager
    filepath = "./model/SuperRetina.om"       #模型的路径
    device_id = 0                            #芯片ID
    model = sdk.model(filepath, device_id)
    ...
    for pair_file in tqdm(match_pairs):
        ...
        H_m1, inliers_num_rate, query_image, _, image_height, image_width = compute_homography(query_im_path, refer_im_path, model)
        # exit()
        H_m2 = None
        if use_matching_trick:
            if H_m1 is not None:
                ...
                H_m2, inliers_num_rate, _, _, image_height, image_width =\
