...
if __name__ == '__main__':
    ...
    for pair_file in tqdm(match_pairs):
        ground_file = os.path.join(gt_dir, pair_file)
        ...
        H_m1, inliers_num_rate, query_image, _, image_height, image_width =\
            cal_homography(query_im_path, refer_im_path, model)
        HM2 = None
        if use_matching_trick:
            if H_m1 is not None:
                ...
                cal_homography(query_align_first, refer_im_path, model, query_is_image=True)
        ...
        if inliers_num_rate < 1e-6:
            ...
            points_gd = np.loadtxt(ground_file)
