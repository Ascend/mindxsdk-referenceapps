...
#include "X3DPostProcess.h"

using namespace std;

namespace MxBase
{
    X3DPostProcess &X3DPostProcess::operator=(const X3DPostProcess &other)
    {
        if (this == &other)
        {
            return *this;
    ...
    APP_ERROR X3DPostProcess::Init(const std::map<std::string, std::shared_ptr<void>> &postConfig)
    {
        LogDebug << "Start to Init X3DPostProcess.";
        ...
        LogDebug << "End to Init X3DPostProcess.";
        return APP_ERR_OK;
    }

    APP_ERROR X3DPostProcess::DeInit()
    {
        return APP_ERR_OK;
    }

    bool X3DPostProcess::IsValidTensors(const std::vector<TensorBase> &tensors) const
    {
        ...
        if (softmaxShape[1] != classNum_)
        {
            LogError << "input size(" << softmaxShape[1] << ") "
                     << "classNumber(" << classNum_ << ")";
            return false;
        }
        return true;
    }

    APP_ERROR X3DPostProcess::Process(const std::vector<TensorBase> &tensors,
                                      std::vector<std::vector<ClassInfo>> &classInfos,
                                      const std::map<std::string, std::shared_ptr<void>> &configParamMap)
    {
        LogDebug << "Start to Process X3DPostProcess.";
        ...
        LogDebug << "End to Process X3DPostProcess.";
        return APP_ERR_OK;
    }
    ...
    extern "C"
    {
        ...
        {
            LogInfo << "Begin to get X3DPostProcess instance.";
            auto instance = std::make_shared<MxBase::X3DPostProcess>();
            LogInfo << "End to get X3DPostProcess instance.";
            return instance;
        }
    }
}