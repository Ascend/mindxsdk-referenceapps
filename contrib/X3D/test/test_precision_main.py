...
with open(args.FRAME_LENGTH_PATH,"r") as fp:
    ...
    for d in data:
        if d=="":
            continue
        idx,frame = d.split()
        frame_length_dict[int(idx)] = int(frame)
...
def test_func(process_id,index_list,cross_process_num,cross_process_Lock):
    ...
    for idx in index_list:
        ...
        p = subprocess.Popen(f'python3.9 test_precision_sub.py --RESULT_SAVE_PATH {args.RESULT_SAVE_PATH} --TEST_VIDEO_IDX {idx} --DEVICE {device_id} --WINDOW_STRIDE {window_stride}', 
