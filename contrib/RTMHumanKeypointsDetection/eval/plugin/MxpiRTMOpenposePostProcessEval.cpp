 ...
 */
#include "MxpiRTMOpenposePostProcessEval.h"
#include <numeric>
#include <algorithm>
    ...
    MxpiRTMOpenposeProtoEval::MxpiPersonList &dstMxpiPersonList)
{
    ...
    for (int k = 0; k < personList.size(); k++) {
        ...
        MxpiRTMOpenposeProtoEval::MxpiMetaHeader *dstPersonMxpiMetaheaderList = mxpiPersonPtr->add_headervec();
...
APP_ERROR MxpiRTMOpenposePostProcess::Process(std::vector<MxpiBuffer*> &mxpiBuffer)
{
    ...
    // Generate output
    shared_ptr<MxpiRTMOpenposeProtoEval::MxpiPersonList> dstMxpiPersonListSptr =
            make_shared<MxpiRTMOpenposeProtoEval::MxpiPersonList>();
