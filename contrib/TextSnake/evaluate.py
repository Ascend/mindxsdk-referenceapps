...
#思路：和main一样的先进行预处理再输入输出，只不过多次循环 每次循环的输出写到一个文件里，文件名是输入图像名.txt,最后调用deteval对这个结果文件夹里的文件进行计算精度
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(FILEPATH):
        ...
        print(IMAGE_PATH1)
    #     cv2.imwrite(IMAGE_PATH1, image, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
    #     # binpath=IMAGE_PATH1.replace('jpg', 'bin')
    #     # image.tofile(binpath)
    #     # print(binpath)
    ...
    #     infer = streamManagerApi.GetResult(STREAMNAME, b'appsink0', keyVec)
        ...
        write_to_file(contours, os.path.join(cfg.output_dir, filename.replace('jpg', 'txt')))
    ...
    subprocess.call(['python3.7', './Deteval.py', "test", '--tr', '0.7', '--tp', '0.6'])
    subprocess.call(['python3.7', './Deteval.py', "test", '--tr', '0.8', '--tp', '0.4'])
    print('End.')