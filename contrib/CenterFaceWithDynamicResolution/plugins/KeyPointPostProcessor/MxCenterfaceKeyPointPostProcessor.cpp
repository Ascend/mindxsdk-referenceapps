...
namespace MxBase {
    ...
    APP_ERROR MxCenterfaceKeyPointPostProcessor::Process(
        ...
        for (int i = 0; i < faces.size(); i++) {
            ...
            for(int j=0;j<5;j++){
                vector<float>temp = {faces[i].landmarks[2*j],faces[i].landmarks[2*j+1]};
    ...
    void MxCenterfaceKeyPointPostProcessor::detect(std::vector<void *> &featLayerData, std::vector<FaceInfo>& faces, const ImageInfo  &imgInfo,float scoreThresh, float nmsThresh)
    {
    ...
    void MxCenterfaceKeyPointPostProcessor::nms(std::vector<FaceInfo>& vec_boxs, float nmsthresh,float iouthresh,float sigma,
                                        unsigned int method)
    {
    ...
    void MxCenterfaceKeyPointPostProcessor::decode(float*  heatmap, float*  scale, float*  offset, float*  landmarks, std::vector<FaceInfo>& faces, const ImageInfo &imageinfo,float scoreThresh, float nmsThresh)
    {
    ...
    std::vector<int> MxCenterfaceKeyPointPostProcessor::getIds(float *heatmap, int  h, int w, float thresh)
    {
        std::vector<int> ids;
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
    ...
    void MxCenterfaceKeyPointPostProcessor::squareBox(std::vector<FaceInfo>& faces,const ImageInfo &imageinfo)
    {
