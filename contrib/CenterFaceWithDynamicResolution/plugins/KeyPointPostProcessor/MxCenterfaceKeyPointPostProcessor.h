...
namespace MxBase{
    class MxCenterfaceKeyPointPostProcessor : public MxBase::KeypointPostProcessBase {
    public:
    ...
    private:
        void nms(std::vector<FaceInfo>& vec_boxs,float nmsthresh,float iouthresh,float sigma=0.5,
                 unsigned int method=1);        void decode(float* heatmap,float* scale,float* offset,float* landmarks, std::vector<FaceInfo>&faces,const ImageInfo &imageinfo,float scoreThresh,float nmsThresh);
