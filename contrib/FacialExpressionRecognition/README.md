...
文本分类插件基于 MindXSDK 开发，在晟腾芯片上进行情绪识别，将分类结果在图片上显示。输入一张目标图片，可以判断属于哪个情绪类别。
...
支持的SDK版本为2.0.4，支持的CANN版本为5.0.4。
...
然后由推理模型插件mxpi_tensorinfer、mxpi_objectpostprocessor和mxpi_distributor推理每张图片的目标区域，
再通过媒体数据处理插件mxpi_imagecrop对输入的图片进行裁剪和缩放操作得到目标图片，最后将目标图片喂入推理模型插件mxpi_tensorinfer得到每种情绪类别的得分。
本系统的各模块及功能描述如表1所示：
...
```
### 1.5 技术实现流程图
   本项目实现自然场景下的情绪识别，首先利用目标检测模型采集图片中的目标图像，然后利用情绪识别模型推理情绪类别。整体流程如下图所示。
...
python3 main.py
```
命令执行成功后项目目录下生成目标框和情绪推理结果的测试图片：my_result.jpg。
