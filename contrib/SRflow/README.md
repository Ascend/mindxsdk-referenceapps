...
选取一对八倍下采样前后的png格式图片作为样例(可直接使用div2k验证集中的图片对)，将下采样后的图片命名为test.png，将hr图片(低分辨图像对应的高分辨图像,即下采样前的图像)命名为test_hr.png放入image文件夹中。注意:实际应用中，先有低分辨的图像test.png，经过人工处理将长边放大至2048的图像为test_hr.png，hr图片长边和短边的比例需要与低分辨图像相等,且长边大小必须为2048（模型的输出图像大小固定为2048*2048）。
