...
void MOTConnection::FilterLowThreshold(const g_hungarianhandle &hungarianHandleObj,
                                      ...
                                      std::vector<cv::Point> &matchedTracedDetected, std::vector<MxBase::ObjectInfo> &unmatchedVehicleObjectQueue)
{
    if (objInfos[0].size() > 0) {
        LogDebug << "[frame id = " << 1 << "], trace size =" << traceList_.size() <<
        "detect size = " << objInfos[0].size() << "";
...
//                LogInfo <<"traceList_:"<<traceList_[i].detectInfo.x0<<","<<traceList_[i].detectInfo.y0<<" objInfos:"<<objInfos[0][j].x0<<","<<objInfos[0][j].y0;
        ...
        g_hungarianhandle hungarianHandleObj = {};
...
APP_ERROR MOTConnection::ProcessSort(std::vector<std::vector<MxBase::ObjectInfo>> &objInfos)
{
    LogInfo << "MOTConnection[]: Begin to process data.";
    ...
    LogDebug << "[frame id = " << 1 << "], matched detect size = " <<
