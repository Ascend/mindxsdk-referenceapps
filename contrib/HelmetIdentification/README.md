...
<img src="%E5%AE%89%E5%85%A8%E5%B8%BD%E8%AF%86%E5%88%ABReadme.assets/image4.jpg" alt="image4" style="zoom:80%;" />
...
在编译运行项目前，需要通过环境配置脚本main-env.sh设置环境变量：
...
注：其中SDK安装路径替换为用户的SDK安装路径;install_path替换为开发套件包所在路径。LD_LIBRARY_PATH用以加载开发套件包中llib库。GST_DEBUG用以mxpi_rtspsrc取流地址配置不正确时出现warning日志提示。
...
| 软件名称                | 版本     | 获取方式                                                     |
| ----------------------- | -------- | ------------------------------------------------------------ |
| opencv-python           | 4.5.2.54 | python3.7 -m pip install opencv-python                       |
| ONNX                    | 1.7.0    | pip install onnx==1.7.0                                      |
| helmet_head_person_s.pt | v2.0     | [原项目链接](https://github.com/PeterH0323/Smart_Construction) |
| pytorch                 | 1.5.1    | pytorch官网                                                  |
| torchvision             | 0.6.1    | torchvision官网                                              |



##### 1.2 pt文件转换为onnx文件

获取模型文件helmet_head_person_s.pt。使用原项目中的export.py将pt文件转换为onnx格式文件。运行：

```shell
python3.7 ./models/export.py --weights ./weights/helmet_head_person_s.pt --img 640 --batch 1
```

其中onnx算子版本为opset_version=11。转换完成后上传至服务器源码根目录中。

然后利用附件脚本dy_resize.py修改模型resize算子。该模型含有动态Resize算子（上采样），通过计算维度变化，改为静态算子，不影响模型的精度，运行如下命令：
...
1.mxpi_modelinfer用于加载yolov5安全帽识别模型。该插件包含四个参数，modelPath用于加载om模型文件。labelPath用于加载模型可识别类postProcessLibPath用于加载后处理动态链接库文件。该模块实现NMS等后处理。postProcessConfigPath用于加载后处理所需要的配置文件。本项目使用libMpYOLOv5PostProcessor.so。其配置文件内容如下：              
...
2.mxpi_selectedframe完成视频跳帧。对于输入帧率为24fps输入视频进行每三帧抽一帧进行识别。实现8fps的帧率，其功能实现代码如下：
...
![image-20210811205453234](%E5%AE%89%E5%85%A8%E5%B8%BD%E8%AF%86%E5%88%ABReadme.assets/image1.jpg)

本项目通过mxpi_rtspsrc拉流输入数据，通过两路GetprotoBuf接口输出数据，一路输出带有帧信息的图片数据，一路输出带有帧信息的目标检测框和检测框跟踪信息。推理运行命令如下：

首先通过live555进行推流，进入到live555安装目录下mediaServer路径，上传要推流的视频在本目录下然后推流，输入命令：
...
![image2](%E5%AE%89%E5%85%A8%E5%B8%BD%E8%AF%86%E5%88%ABReadme.assets/image2.jpg)
...
![image3](%E5%AE%89%E5%85%A8%E5%B8%BD%E8%AF%86%E5%88%ABReadme.assets/image3.jpg)
