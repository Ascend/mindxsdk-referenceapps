...
def first_model(img_pathf, intrinsic_fif):
    ...
    for n in range(0, len_img_pathf):
        ...
        inPluginId = 0
        # 根据流名将检测目标传入流中
        uniqueId = stream_one.SendData(streamName, inPluginId, data_input)
        if uniqueId < 0:
        ...
        # 从流中取出对应插件的输出数据
        infer_result = stream_one.GetProtobuf(streamName, 0, keyVec)
...
if __name__ == '__main__':
    ...
    with os.fdopen(os.open(output_pathh, flags, modes), 'w') as fw:
