...
系统实现流程参见图1.1：![1.1](C:\Users\13089\Desktop\HW_JF\交付件\img\1.1.png)
...
**步骤4** 在[https://mindx.sdk.obs.cn-north-4.myhuaweicloud.com/mindxsdk-referenceapps%20/contrib/3DMPPE-ROOTNET/mupots-3d-eval.zip](https://vcai.mpi-inf.mpg.de/projects/SingleShotMultiPerson/)下载使用的MuPoTS数据集，包括MultiPersonTestSet以及MuPoTS-3D.json（标注），将一张图片放在pic文件夹下。然后在untils中找到ini.py并运行来生成需要的文件数据。
...
**步骤6** 精度验证。将整个MultiPersonTestSet数据集放在pic文件夹下。然后在untils中找到ini.py并运行来生成需要的文件数据。运行eval_pic.py来生成bbox_root_mupots_output.json，并将其和MuPoTS-3D.json放在项目的eval目录下，进行精度验证，执行命令：
