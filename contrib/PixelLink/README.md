...
## 1 介绍
  ...
  本项目试用场景为：包含文字区域的图像，要求文字区域尽可能清晰，区域大小能够占图像尺寸的5%及以上最佳。图像中文本区域要不存在遮挡、稀疏的密集度、文本要求最好为汉字或者英文。若文本区域存在遮挡、文本区域过于密集以及文本为特殊符号等可能存在无法检测出结果的问题。对于输入图像，图像要求为768x1280x3的彩色图像。图像中全部的文字区域不清晰、大小不支持识别或者不存在文字区域时，识别可能会出现问题，最后可视化即为原图。本项目在运行代码后，会根据测试图像生成一个txt文件，其中包含文字区域位置的点坐标，每一个文字区域由四个坐标点组成。最后会根据txt文件中的每一组坐标点在图像上绘制出文本区域的线条框。线条框并非规则矩形，而是包含文本区域的连线。
