...
def addCatItem(name):
    ...
    category_item_id = labels_map.index(name)
...
def addAnnoItem(object_name, image_id, category_id, bbox, ignore):
...
def parseXmlFiles_by_txt(data_dir,json_save_path,split='trainval'):
    print("hello")
    labelfile=split+".txt"
    ...
    for _id in ids:
        ...
        for elem in root:
            ...
            # add img item only after parse <size> tag
            if elem.tag != 'filename' and elem.tag != 'folder' and current_image_id is None and file_name is not None and size['width'] is not None:
                if file_name not in image_set:
                    current_image_id = addImgItem(file_name, size)
                    print('add image with {} and {}'.format(file_name, size))
                else:
            ...
            for subelem in elem:
                if current_parent == 'object':
                    if subelem.tag == 'difficult': # or subelem.tag == 'truncated':
                        ignore = int(subelem.text)
            ...
            for subelem in elem:
                ...
                if bndbox['xmin'] is not None:
                    ...
                    print('add annotation with {},{},{},{}'.format(object_name, current_image_id, current_category_id,
                                                                   bbox))
                    addAnnoItem(object_name, current_image_id, current_category_id, bbox, ignore)
...
if __name__ == '__main__':
    ...
    json_save_path="./VOCdevkit/voc2012val.json"
