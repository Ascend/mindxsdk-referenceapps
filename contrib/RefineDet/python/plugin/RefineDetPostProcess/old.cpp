...
namespace {
    ...
    const float IMAGE_WIDTH = 1024.0;
    const float IMAGE_HEIGHT = 1024.0;
    //const float STEPS[3] = {32.0, 64.0, 128.0};
    const float STEPS[4] = {8.0, 16.0, 32.0, 64.0};
    const float VARIANCE[2] = {0.1, 0.2};
    const uint32_t in_shape = 60160;
    const uint32_t topK = 5000;
}
namespace MxBase {
...
void YunetPostProcess::ObjectDetectionOutput(const std::vector <TensorBase>& tensors,
                                                 std::vector <std::vector<ObjectInfo>>& objectInfos, const std::vector <ResizedImageInfo>& resizedImageInfos)
    {
        ...
        //LogInfo << shape[0] << " " << shape[1] << " " <<shape[2];
        ...
        uint32_t VectorNum = in_shape<topK ? in_shape : in_shape;
        ...
        for (int i = 0; i < f[0]; i++){
            for (int j = 0; j < f[1]; j++) {
                for (auto min_size : _min_sizes) {
                    ...
                    //anchor.at<float>(0,0) 
                    float xmin = (center_x - (float)min_size / 2.f) / IMAGE_SIZE;
                    float ymin = (center_y - (float)min_size / 2.f) / IMAGE_SIZE;
                    float xmax = (center_x + (float)min_size / 2.f) / IMAGE_SIZE;
                    float ymax = (center_y + (float)min_size / 2.f) / IMAGE_SIZE;
